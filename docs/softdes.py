...
def converte_data(orig):
    "Muda a ordem de chunks de uma string de entrada"
    return orig[8:10]+'/'+orig[5:7]+'/'+orig[0:4]+' '+orig[11:13]+':'+orig[14:16]+':'+orig[17:]

def get_quizes(user):
    "Resgata os todos os quizes de um usuario"
...
def get_user_quiz(userid, quizid):
    "Resgata um quiz expecifico de um usuario na tabela USERQUIZ"
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
...
def set_user_quiz(userid, quizid, sent, answer, result):
    "Designa um quiz para um aluno com sua resposta e outras informacoes"
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
...
def get_quiz(id_q, user):
    "Resgata um quiz expecifico de um usuario na tabela QUIZ"
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
    if user in ('admin', 'fabioja'):
        cursor.execute("SELECT id, release, expire, problem, tests, results,\
             diagnosis, numb from QUIZ where id = {0}".format(id_q))
    else:
        cursor.execute("SELECT id, release, expire, problem, tests, results,\
             diagnosis, numb from QUIZ where id = {0} and release < '{1}'"\
                 .format(id_q, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    info = [reg for reg in cursor.fetchall()]
    conn.close()
    return info

def set_info(pwd, user):
    "Designa as credenciais a um usuario"
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
...
def get_info(user):
    "Resgata as crendenciais de um usuario"
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
...
def main():
    ...
    if request.method == 'POST' and 'ID' in request.args:
        ...
        if len(quiz) == 0:
            ...
            return render_template('index.html', username=AUTH.username(), \
                challenges=challenges, p=p, msg=msg)
        quiz = quiz[0]
        if sent > quiz[2]:
        ...
        args = {"ndes": id, "code": answer, "args": eval(quiz[4]), \
            "resp": eval(quiz[5]), "diag": eval(quiz[6])}
        #response = lamb.invoke(FunctionName="Teste",\
        #  InvocationType='RequestResponse', Payload=json.dumps(args))
    ...
    return render_template('index.html', username=AUTH.username(),\
        ...
        e=(sent > quiz[0][2]), answers=answers, p=p, msg=msg,\
